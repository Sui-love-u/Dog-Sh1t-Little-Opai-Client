...
@JNICInclude
/* loaded from: Arcane 8.10.jar:qwq/arcane/module/Mine.class */
public class Mine implements IThreadListener, IPlayerUsage {
    ...
    private static final Lock threadLock;
    ...
    private long field_175615_aJ = 0;
...
//                }
//                threadLock.unlock();
//                initializeGame(gameConfig); // 提取初始化逻辑到方法
//            } catch (InterruptedException e) {
//                logger.debug("Game thread interrupted");
//                threadLock.unlock();
//                initializeGame(gameConfig); // 在catch块中调用初始化方法
//            }
      //  } catch (Throwable th) {
        //    threadLock.unlock();
            initializeGame(gameConfig); // 在finally块中调用初始化方法
          //  throw th;
       // }
    }

    private void initializeGame(GameConfiguration gameConfig) {
        theMine = this;
    ...
    private void startGame() throws InterruptedException, IOException, LWJGLException {
//        if (ClientApplication.Hwid) {
//            JOptionPane.showMessageDialog(new ClientApplication(), "你还想破解你爹呢 我反反复复抽查你妈 我给你吗操的倒立螺旋升天", "破解你妈逼", 1);
//            System.exit(0);
//        }
...
//        }
        ...
        AchievementList.openInventory.setStatStringFormatter(str -> {
            try
            {
                return String.format(str, GameSettings.getKeyDisplayString(Mine.this.gameSettings.keyBindInventory.getKeyCode()));
            }
        ...
        if (this.serverName != null) {
            displayGuiScreen(new GuiConnecting(new MainMenu(), this, this.serverName, this.serverPort));
        } else {
    ...
    private void createDisplay() throws InterruptedException, LWJGLException {
    ...
    public void displayCrashReport(CrashReport crashReportIn) {
        ...
        Client.Instance.getConfigManager();
        ConfigManager.saveConfig("config", Client.INSTANCE.getModuleManager());
    ...
    public void draw(int posX, int posY, int texU, int texV, int width, int height, int red, int green, int blue, int alpha) {
        ...
        worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        worldrenderer.pos(posX, posY + height, 0.0d).tex(texU * 0.00390625f, (texV + height) * 0.00390625f).color(red, green, blue, alpha).endVertex();
        worldrenderer.pos(posX + width, posY + height, 0.0d).tex((texU + width) * 0.00390625f, (texV + height) * 0.00390625f).color(red, green, blue, alpha).endVertex();
        worldrenderer.pos(posX + width, posY, 0.0d).tex((texU + width) * 0.00390625f, texV * 0.00390625f).color(red, green, blue, alpha).endVertex();
        worldrenderer.pos(posX, posY, 0.0d).tex(texU * 0.00390625f, texV * 0.00390625f).color(red, green, blue, alpha).endVertex();
    ...
    public void shutdownMinecraftApplet() {
        try {
            ScaledResolution sr = new ScaledResolution(Instance.mc);
    ...
    private void runGameLoop() throws IllegalAccessException, IOException, IllegalArgumentException, LWJGLException, InvocationTargetException {
        ...
        this.mcProfiler.endSection();
        if (!this.skipRenderWorld && !TickBase.INSTANCE.freezeAnimation()) {
    ...
    private long runTicks() throws IllegalAccessException, IOException, IllegalArgumentException, LWJGLException, InvocationTargetException {
        new Disabler();
        ...
        for (int j = 0; j < this.timer.elapsedTicks; j++) {
            if (Disabler.getGrimPost()) {
                ...
                if (TickBase.INSTANCE.handleTick()) {
                    continue;
                } else {
                    runTick();
                    if (this.lastTickSentC03) {
                        continue;
                    } else {
                        Queue<FutureTask<?>> queue2 = this.scheduledTasks;
                        synchronized (queue2) {
                            while (!this.scheduledTasks.isEmpty()) {
                                Util.runTask(this.scheduledTasks.poll(), logger);
                            }
    ...
    private void updateDebugProfilerName(int keyCount) {
        ...
        if (list != null && !list.isEmpty()) {
            ...
            int keyCount2 = keyCount - 1;
            if (keyCount2 < list.size() && !list.get(keyCount2).field_76331_c.equals("unspecified")) {
                if (this.debugProfilerName.length() > 0) {
                    this.debugProfilerName += ".";
                }
                this.debugProfilerName += list.get(keyCount2).field_76331_c;
            }
        }
    }
    ...
    private void displayDebugInfo(long elapsedTicksTime) {
        ...
        if (this.mcProfiler.profilingEnabled) {
            ...
            int k = this.displayHeight - (160 * 2);
            GlStateManager.enableBlend();
            worldrenderer.begin(7, DefaultVertexFormats.POSITION_COLOR);
            worldrenderer.pos(j - (160 * 1.1f), (k - (160 * 0.6f)) - 16.0f, 0.0d).color(200, 0, 0, 0).endVertex();
            worldrenderer.pos(j - (160 * 1.1f), k + (160 * 2), 0.0d).color(200, 0, 0, 0).endVertex();
            worldrenderer.pos(j + (160 * 1.1f), k + (160 * 2), 0.0d).color(200, 0, 0, 0).endVertex();
            worldrenderer.pos(j + (160 * 1.1f), (k - (160 * 0.6f)) - 16.0f, 0.0d).color(200, 0, 0, 0).endVertex();
            tessellator.draw();
            ...
            for (int l = 0; l < list.size(); l++) {
                ...
                for (int j2 = i1; j2 >= 0; j2--) {
                    float f = (float) ((((d0 + ((profiler$result1.field_76332_a * j2) / i1)) * 3.141592653589793d) * 2.0d) / 100.0d);
                    float f1 = MathHelper.sin(f) * 160;
                    float f2 = MathHelper.cos(f) * 160 * 0.5f;
                    worldrenderer.pos(j + f1, k - f2, 0.0d).color(k1, l1, i2, 255).endVertex();
                }
                tessellator.draw();
                worldrenderer.begin(5, DefaultVertexFormats.POSITION_COLOR);
                for (int i3 = i1; i3 >= 0; i3--) {
                    float f3 = (float) ((((d0 + ((profiler$result1.field_76332_a * i3) / i1)) * 3.141592653589793d) * 2.0d) / 100.0d);
                    float f4 = MathHelper.sin(f3) * 160;
                    float f5 = MathHelper.cos(f3) * 160 * 0.5f;
            ...
            if (profiler$result.field_76331_c.length() == 0) {
                ...
                s = s2 + profiler$result.field_76331_c + " ";
            }
            this.fontRendererObj.drawStringWithShadow(s, j - 160, (k - (160 / 2)) - 16, 16777215);
            FontRenderer fontRenderer = this.fontRendererObj;
            String s3 = decimalformat.format(profiler$result.field_76330_b) + "%";
            fontRenderer.drawStringWithShadow(s3, (j + 160) - this.fontRendererObj.getStringWidth(s3), (k - (160 / 2)) - 16, 16777215);
            for (int k2 = 0; k2 < list.size(); k2++) {
                ...
                this.fontRendererObj.drawStringWithShadow(s1 + profiler$result2.field_76331_c, j - 160, k + (160 / 2) + (k2 * 8) + 20, profiler$result2.getColor());
                FontRenderer fontRenderer2 = this.fontRendererObj;
                String s12 = decimalformat.format(profiler$result2.field_76332_a) + "%";
                fontRenderer2.drawStringWithShadow(s12, ((j + 160) - 50) - this.fontRendererObj.getStringWidth(s12), k + (160 / 2) + (k2 * 8) + 20, profiler$result2.getColor());
                FontRenderer fontRenderer3 = this.fontRendererObj;
                String s13 = decimalformat.format(profiler$result2.field_76330_b) + "%";
                fontRenderer3.drawStringWithShadow(s13, (j + 160) - this.fontRendererObj.getStringWidth(s13), k + (160 / 2) + (k2 * 8) + 20, profiler$result2.getColor());
            }
        }
    }
            ...
            try {
                ...
                if (this.currentScreen != null) {
                    try {
                        ...
                        crashreportcategory1.addCrashSectionCallable("Screen name", new Callable<String>() { // from class: qwq.arcane.module.Mine.4
                            /* JADX WARN: Can't rename method to resolve collision */
                            @Override // java.util.concurrent.Callable
                            public String call() throws Exception {
                                return Mine.this.currentScreen.getClass().getCanonicalName();
                            }
                        });
                ...
                crashreportcategory.addCrashSectionCallable("Screen name", new Callable<String>() { // from class: qwq.arcane.module.Mine.3
                    /* JADX WARN: Can't rename method to resolve collision */
                    @Override // java.util.concurrent.Callable
                    public String call() throws Exception {
                        return Mine.this.currentScreen.getClass().getCanonicalName();
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
                    ...
                    if (j != 0) {
                        if (this.thePlayer.isSpectator()) {
                            ...
                            if (this.ingameGUI.getSpectatorGui().func_175262_a()) {
                                ...
                                float f = MathHelper.clamp_float(this.thePlayer.capabilities.getFlySpeed() + (j2 * 0.005f), 0.0f, 0.2f);
            ...
            while (Keyboard.next()) {
                ...
                if (this.debugCrashKeyPressTime > 0) {
                    if (getSystemTime() - this.debugCrashKeyPressTime >= 6000) {
            ...
            if (!event.isCancelled()) {
                if (this.thePlayer.isUsingItem()) {
                    ...
                    while (this.gameSettings.keyBindAttack.isPressed()) {
                        if (((Animations) Client.Instance.getModuleManager().getModule(Animations.class)).getState() && ((Animations) Client.Instance.getModuleManager().getModule(Animations.class)).getSwingWhileUsingItem().get().booleanValue() && (!this.thePlayer.isSwingInProgress || this.thePlayer.swingProgressInt >= this.thePlayer.getArmSwingAnimationEnd() / 2 || this.thePlayer.swingProgressInt < 0)) {
    ...
    public static boolean isGuiEnabled() {
        return theMine == null || !theMine.gameSettings.hideGUI;
    }

    public static boolean isFancyGraphicsEnabled() {
        return theMine != null && theMine.gameSettings.fancyGraphics;
    }

    public static boolean isAmbientOcclusionEnabled() {
        return (theMine == null || theMine.gameSettings.ambientOcclusion == 0) ? false : true;
    }
    ...
    public CrashReport addGraphicsAndWorldToCrashReport(CrashReport theCrash) {
        theCrash.getCategory().addCrashSectionCallable("Launched Version", new Callable<String>() { // from class: qwq.arcane.module.Mine.5
            /* JADX WARN: Can't rename method to resolve collision */
            @Override // java.util.concurrent.Callable
            public String call() throws Exception {
                return Mine.this.launchedVersion;
            }
        });
        theCrash.getCategory().addCrashSectionCallable("LWJGL", new Callable<String>() { // from class: qwq.arcane.module.Mine.6
            /* JADX WARN: Can't rename method to resolve collision */
            @Override // java.util.concurrent.Callable
            public String call() {
        ...
        theCrash.getCategory().addCrashSectionCallable("OpenGL", new Callable<String>() { // from class: qwq.arcane.module.Mine.7
            /* JADX WARN: Can't rename method to resolve collision */
            @Override // java.util.concurrent.Callable
            public String call() {
        ...
        theCrash.getCategory().addCrashSectionCallable("GL Caps", new Callable<String>() { // from class: qwq.arcane.module.Mine.8
            /* JADX WARN: Can't rename method to resolve collision */
            @Override // java.util.concurrent.Callable
            public String call() {
        ...
        theCrash.getCategory().addCrashSectionCallable("Using VBOs", new Callable<String>() { // from class: qwq.arcane.module.Mine.9
            /* JADX WARN: Can't rename method to resolve collision */
            @Override // java.util.concurrent.Callable
            public String call() {
                return Mine.this.gameSettings.useVbo ? "Yes" : "No";
            }
        });
        theCrash.getCategory().addCrashSectionCallable("Is Modded", new Callable<String>() { // from class: qwq.arcane.module.Mine.10
            /* JADX WARN: Can't rename method to resolve collision */
            @Override // java.util.concurrent.Callable
            public String call() throws Exception {
                String s = ClientBrandRetriever.getClientModName();
                return !s.equals(CustomColormap.FORMAT_VANILLA_STRING) ? "Definitely; Client brand changed to '" + s + "'" : Mine.class.getSigners() == null ? "Very likely; Jar signature invalidated" : "Probably not. Jar signature remains and client brand is untouched.";
            }
        });
        theCrash.getCategory().addCrashSectionCallable("Type", new Callable<String>() { // from class: qwq.arcane.module.Mine.11
            /* JADX WARN: Can't rename method to resolve collision */
            @Override // java.util.concurrent.Callable
            public String call() throws Exception {
        ...
        theCrash.getCategory().addCrashSectionCallable("Resource Packs", new Callable<String>() { // from class: qwq.arcane.module.Mine.12
            /* JADX WARN: Can't rename method to resolve collision */
            @Override // java.util.concurrent.Callable
            public String call() throws Exception {
                StringBuilder stringbuilder = new StringBuilder();
                for (String s : Mine.this.gameSettings.resourcePacks) {
                    if (stringbuilder.length() > 0) {
                        stringbuilder.append(", ");
                    }
                    stringbuilder.append(s);
                    if (Mine.this.gameSettings.incompatibleResourcePacks.contains(s)) {
        ...
        theCrash.getCategory().addCrashSectionCallable("Current Language", new Callable<String>() { // from class: qwq.arcane.module.Mine.13
            /* JADX WARN: Can't rename method to resolve collision */
            @Override // java.util.concurrent.Callable
            public String call() throws Exception {
                return Mine.this.mcLanguageManager.getCurrentLanguage().toString();
            }
        });
        theCrash.getCategory().addCrashSectionCallable("Profiler Position", new Callable<String>() { // from class: qwq.arcane.module.Mine.14
            /* JADX WARN: Can't rename method to resolve collision */
            @Override // java.util.concurrent.Callable
            public String call() throws Exception {
                return Mine.this.mcProfiler.profilingEnabled ? Mine.this.mcProfiler.getNameOfLastSection() : "N/A (disabled)";
            }
        });
        theCrash.getCategory().addCrashSectionCallable("CPU", new Callable<String>() { // from class: qwq.arcane.module.Mine.15
            /* JADX WARN: Can't rename method to resolve collision */
            @Override // java.util.concurrent.Callable
            public String call() {
    ...
    public static Mine getMinecraft() {
        return theMine;
    }

    public ListenableFuture<Object> scheduleResourcesRefresh() {
        return addScheduledTask(new Runnable() { // from class: qwq.arcane.module.Mine.16
            @Override // java.lang.Runnable
            public void run() {
                Mine.this.refreshResources();
            }
        });
    }
    ...
    public void addServerStatsToSnooper(PlayerUsageSnooper playerSnooper) {
        ...
        playerSnooper.addClientStat("run_time", Long.valueOf(((MinecraftServer.getCurrentTimeMillis() - playerSnooper.getMinecraftStartTimeMillis()) / 60) * 1000));
    ...
    public static int getGLMaximumTextureSize() {
        int i = CustomItems.MASK_POTION_SPLASH;
        while (true) {
            int i2 = i;
            if (i2 > 0) {
    ...
    public static void stopIntegratedServer() {
        ...
        if (theMine != null && (integratedserver = theMine.getIntegratedServer()) != null) {
                        ...
                        else if (this.getTwitchStream().isReadyToBroadcast())
                        {
                            this.displayGuiScreen(new GuiYesNo(new GuiYesNoCallback()
                            {
                                public void confirmClicked(boolean result, int id)
                                {
                                    if (result)
                                    {
                                        Mine.this.getTwitchStream().func_152930_t();
                                    }

                                    Mine.this.displayGuiScreen((GuiScreen)null);
                                }
    ...
    public static Map<String, String> getSessionInfo() {
        ...
        map.put("X-Minecraft-Version", Config.MC_VERSION);
        return map;
    }
