...
public class CustomChunkMethod {
    ...
    public static int processByte(int n2, long l) {
        return n2 ^ field_3603[(int)(l % (long)field_3603.length)];
    }

    public static void processData(byte[] byArray, long l) {
        for (int i = 0; i < byArray.length; ++i) {
            int n2 = i;
            byArray[n2] = (byte)(byArray[n2] ^ field_3603[(int)(((long)i + l) % (long)field_3603.length)]);
        }
    }

    public static void processData(byte[] byArray, long l, int n2, int n22) {
        for (int i = 0; i < n22; ++i) {
            int n3 = i + n2;
