...
public abstract class Entity implements ICommandSender
{
    ...
    public com.amaya.utils.player.Rise.Vec3 getPositionEyes2(float partialTicks) {
        if (partialTicks == 1.0F) {
            return new com.amaya.utils.player.Rise.Vec3(this.posX, this.posY + (double) this.getEyeHeight(), this.posZ);
        } else {
            double d0 = this.prevPosX + (this.posX - this.prevPosX) * (double) partialTicks;
            double d1 = this.prevPosY + (this.posY - this.prevPosY) * (double) partialTicks + (double) this.getEyeHeight();
            double d2 = this.prevPosZ + (this.posZ - this.prevPosZ) * (double) partialTicks;
            return new com.amaya.utils.player.Rise.Vec3(d0, d1, d2);
        }
    }
}
