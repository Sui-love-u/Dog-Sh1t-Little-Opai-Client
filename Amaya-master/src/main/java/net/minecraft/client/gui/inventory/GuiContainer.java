...
import com.amaya.module.impl.player.ChestStealer;
...
public abstract class GuiContainer extends GuiScreen
{
    ...
    {
        if (Amaya.Instance.moduleManager.getModule(ChestStealer.class).silentValue.get() && Amaya.Instance.moduleManager.getModule(ChestStealer.class).getState()){
            GuiScreen guiScreen = mc.currentScreen;
            if (guiScreen instanceof GuiChest chest) {
                if (chest.lowerChestInventory != null) {
                    mc.setIngameFocus();
                    mc.currentScreen = guiScreen;
                    RenderUtil.drawLoadingCircle(this.width / 2,(float) this.height / 2.0F);
                    FontManager.REGULAR.get(18).drawString("Stealing...",this.width / 2 - FontManager.REGULAR.get(18).getStringWidth("Stealing...") / 2,(float) this.height / 2.0F + 40.0F,-1);
                    return;
                }
            }
