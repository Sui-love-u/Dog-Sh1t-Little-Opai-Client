...
public class NetHandlerPlayClient
    ...
    public void handleJoinGame(S01PacketJoinGame packetIn) {
        ...
        if (Amaya.Instance.moduleManager.getModule(Protocol.class).getState() && Protocol.mode.is("Quick Macro") && ((Boolean)Protocol.bypassGreen.get()).booleanValue()) {
            PacketUtil.sharkBee();
        }
    }
    ...
    public void handleMapChunkBulk(S26PacketMapChunkBulk packetIn) {
        ...
        for (int i = 0; i < packetIn.getChunkCount(); ++i) {
            ...
            this.clientWorldController.doPreChunk(j2, k, true);
            this.clientWorldController.invalidateBlockReceiveRegion(j2 << 4, 0, k << 4, (j2 << 4) + 15, 256, (k << 4) + 15);
            Chunk chunk = this.clientWorldController.getChunkFromChunkCoords(j2, k);
            chunk.fillChunk(packetIn.getChunkBytes(i), packetIn.getChunkSize(i), true);
            this.clientWorldController.markBlockRangeForRenderUpdate(j2 << 4, 0, k << 4, (j2 << 4) + 15, 256, (k << 4) + 15);
    ...
    public void handleTeams(S3EPacketTeams packetIn) {
        ...
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        Scoreboard scoreboard = this.clientWorldController.getScoreboard();
        ScorePlayerTeam scorePlayerTeam = scoreplayerteam = packetIn.getAction() == 0 ? scoreboard.createTeam(packetIn.getName()) : scoreboard.getTeam(packetIn.getName());
