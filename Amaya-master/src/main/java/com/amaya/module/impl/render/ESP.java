...
import com.amaya.events.impl.render.RenderNameTagEvent;
...
import static com.amaya.utils.render.RenderUtil.color;
...
public class ESP extends Module {
    public final BooleanSetting tags = new BooleanSetting("Tags", true);
    public final BooleanSetting fontTags = new BooleanSetting("Font Tags", true);
    public final BooleanSetting fonttagsBackground = new BooleanSetting("Font Tags Background", fontTags::get, true);
    public final BooleanSetting fonttagsHealth = new BooleanSetting("Font Tags Health", fontTags::get, true);
    public final NumberSetting tagsSize = new NumberSetting("Tags Size", tags::get,0.5f, 0.1f, 2, 0.05f);
    public final BooleanSetting tagsHealth = new BooleanSetting("Tags Health", tags::get,true);
    public final BooleanSetting tagsBackground = new BooleanSetting("Tags Background", tags::get,true);
    public final BooleanSetting tagsHealthBar = new BooleanSetting("Tags Health Bar", tags::get, true);
    public final BooleanSetting item = new BooleanSetting("Item", tags::get,true);
    public final BooleanSetting esp2d = new BooleanSetting("2D ESP", true);
    public final BooleanSetting box = new BooleanSetting("Box", esp2d::get, true);
    public final BooleanSetting boxSyncColor = new BooleanSetting("Box Sync Color", () -> esp2d.get() && box.get(), false);
    public final ColorSetting boxColor = new ColorSetting("Box Color", () -> esp2d.get() && box.get() && !boxSyncColor.get(), Color.RED);
    public final BooleanSetting healthBar = new BooleanSetting("Health Bar", esp2d::get, true);
    public final BooleanSetting healthBarSyncColor = new BooleanSetting("Health Bar Sync Color", () -> esp2d.get() && healthBar.get(),false);
    public final ColorSetting absorptionColor = new ColorSetting("Absorption Color", () -> esp2d.get() && healthBar.get() && !healthBarSyncColor.get(), new Color(255, 255, 50));
    public final BooleanSetting armorBar = new BooleanSetting("Armor Bar", esp2d::get,true);
    public final ColorSetting armorBarColor = new ColorSetting("Armor Bar Color", () -> esp2d.get() && armorBar.get(), new Color(50, 255, 255));
    public final BooleanSetting skeletons = new BooleanSetting("Skeletons", esp2d::get,true);
    public final NumberSetting skeletonWidth = new NumberSetting("Skeletons Width", () -> esp2d.get() && skeletons.get(), 0.5f, 0.5f, 5, 0.5f);
    public final ColorSetting skeletonsColor = new ColorSetting("Skeletons Color", () -> esp2d.get() && skeletons.get(), Color.WHITE);
    ...
    public void onRender2D(Render2DEvent event) {
        for (EntityPlayer player : entityPosMap.keySet()) {
            ...
            if (tags.get()) {
                ...
                final String name = player.getDisplayName().getFormattedText() + healthString;
                float halfWidth = (float) ((float) fontRenderer.getStringWidth(name) / 2 * tagsSize.get());
                final float xDif = x2 - x;
                final float middle = x + (xDif / 2);
                final float textHeight = (float) (fontRenderer.FONT_HEIGHT * tagsSize.get());
                float renderY = y - textHeight - 2;
                ...
                if (item.get()) {
                    ...
                    float armorX = (float) (middle - ((float) (items.size() * 18) / 2) * tagsSize.get());

                    for (ItemStack stack : items) {
                        RenderUtil.renderItemStack(stack, (double) armorX, renderY -  25 * tagsSize.get(), (float) (tagsSize.get() + tagsSize.get() / 2), true);
            ...
            if (esp2d.get()) {
                ...
                if (healthBar.get()) {
                    ...
                    if (healthBarSyncColor.get()) {
                        final int syncedcolor = Amaya.Instance.moduleManager.getModule(HUD.class).FirstColor.get().getRGB();
                    ...
                    if (healthBarSyncColor.get()) {
                        RenderUtil.color(Amaya.Instance.moduleManager.getModule(HUD.class).FirstColor.get().getRGB());
                    } else {
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        if (skeletons) {
            ...
            RenderUtil.color(skeletonsColor.get().getRGB());
    ...
    private void drawSkeleton(float pt, EntityPlayer player) {
        ...
        if ((entPos = playerRotationMap.get(player)) != null) {
            ...
            final Double xOff = MathUtils.interpolate(player.prevRenderYawOffset, player.renderYawOffset, pt);
            float yOff = sneaking ? 0.6F : 0.75F;
            glRotatef((float) -xOff, 0.0F, 1.0F, 0.0F);
            ...
            glRotatef((float) (xOff - player.rotationYawHead), 0.0F, 1.0F, 0.0F);
    ...
    public BooleanSetting getFontTags() {
        return fontTags;
    }
}
