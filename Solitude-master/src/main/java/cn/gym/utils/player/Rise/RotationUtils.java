...
import cn.gym.utils.math.Vector3d;
...
public final class RotationUtils {
	...
	public static Vector2f calculate(final net.minecraft.util.Vec3 to) {
		...
		for (double xSearch = 0.1D; xSearch < 0.9D; xSearch += 0.1D) {
			for (double ySearch = 0.1D; ySearch < 0.9D; ySearch += 0.1D) {
				for (double zSearch = 0.1D; zSearch < 0.9D; zSearch += 0.1D) {
					...
					final MovingObjectPosition obj = mc.theWorld.rayTraceBlocks(new Vec3(eyesPos.getX(),eyesPos.getY(),eyesPos.getZ()), new Vec3(vector.getX(),vector.getY(),vector.getZ()), false,
							false, true);
	...
	public static boolean isVisible(final Vec3 vec3) {
		...
		return mc.theWorld.rayTraceBlocks(eyesPos, new Vec3(vec3.getX(),vec3.getY(),vec3.getZ())) == null;
	}
