...
public class ProjectileUtil implements Instance {
    public static ProjectileHit predict(double posX, double posY, double posZ, double motionX, double motionY, double motionZ, double motionSlowdown, double size, double gravity, boolean draw) {
        ...
        while (!hasLanded && posY > -60.0) {
            ...
            List<Entity> entityList = mc.theWorld.getEntitiesWithinAABB(Entity.class, arrowBox.addCoord(motionX, motionY, motionZ).expand(1.0, 1.0, 1.0));
            for (int i = 0; i < entityList.size(); ++i) {
                AxisAlignedBB var2;
                MovingObjectPosition possibleEntityLanding;
                Entity var18 = entityList.get(i);
                if (!var18.canBeCollidedWith() || var18 == mc.thePlayer || (possibleEntityLanding = (var2 = var18.getEntityBoundingBox().expand(size, size, size)).calculateIntercept(posBefore, posAfter)) == null) continue;
                hitEntity = true;
                hasLanded = true;
            ...
            if (!draw) continue;
            mc.getRenderManager();
            double d = posX - RenderManager.getRenderPosX();
            mc.getRenderManager();
            double d2 = posY - RenderManager.getRenderPosY();
            mc.getRenderManager();
            GL11.glVertex3d(d, d2, posZ - RenderManager.getRenderPosZ());
        }
    ...
    public static class EnderPearlPredictor {
            ...
            for (double rate = 0.0; rate <= 1.0; rate += 0.3333) {
                for (int yaw = -1; yaw <= 1; ++yaw) {
                    for (int pitch = -1; pitch <= 1; ++pitch) {
                        mul *= this.assessSingleRotation(new Vector2f(rotation.getX() + (float)yaw * 0.5f, rotation.getY() + (float)pitch * 0.5f), MathUtils.interpolate(this.minMotionY, this.maxMotionY, rate));
                        ++cnt;
                    }
                }
        ...
        private double assessSingleRotation(Vector2f rotation, double motionYOffset) {
            ...
            double posY = this.predictY + (double)mc.thePlayer.getEyeHeight() - (double)0.1f;
