...
public class StringComponent extends Component {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        RoundedUtil.drawRoundOutline(getX() + 106, getY() + 13, 90, 16, 2, .1f, new Color(ColorUtil.interpolateColor2(new Color(31, 31, 31), new Color(47, 47, 47), (float) input.getOutput().floatValue())), new Color(ColorUtil.interpolateColor2(
                new Color(31, 31, 31), new Color(47, 47, 47), (float) input.getOutput().floatValue())));
        drawTextWithLineBreaks(textToDraw + (inputting && text.length() < 59 && System.currentTimeMillis() % 1000 > 500 ? "|" : ""), getX() + 108, getY() + 19, 90);
    ...
    private void drawTextWithLineBreaks(String text, float x, float y, float maxWidth) {
        ...
        for (String line : lines) {
            ...
            for (String wrappedLine : wrappedLines) {
                ...
                Semibold.get(16).drawString(wrappedLine, x, currentY, ColorUtil.interpolateColor2(new Color(-1).darker(),
    ...
    private List<String> wrapText(String text, float size, float maxWidth) {
        ...
        for (String word : words) {
            if (Semibold.get(16).getStringWidth(word) <= maxWidth) {
                if (Semibold.get(16).getStringWidth(currentLine.toString() + word) <= maxWidth) {
    ...
    private StringBuilder breakAndAddWord(String word, StringBuilder currentLine, float maxWidth, List<String> lines) {
        ...
        for (int i = 0; i < wordLength; i++) {
            ...
            if (Semibold.get(16).getStringWidth(nextPart) <= maxWidth) {
