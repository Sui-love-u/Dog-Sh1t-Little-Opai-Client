...
public class ColorPickerComponent extends Component {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        this.setHeight((float) (FontManager.Regular.get(15).getHeight() + ((FontManager.Regular.get(15).getHeight() + 2 + 45 + 2 + 12) * open.getOutput())));

        final float[] hsb = new float[]{setting.getHue(), setting.getSaturation(), setting.getBrightness()};
        final float alpha = setting.getAlpha();

        FontManager.Regular.get(15).drawString(setting.getName(), getX() + 4, getY(), -1);
        RoundedUtil.drawRound(getX() + getWidth() - 18, getY(), 15, FontManager.Regular.get(15).getHeight() - 3, 2, setting.get());

        if (opened) {

            RoundedUtil.drawGradientRound(getX() + 2, getY() + FontManager.Regular.get(15).getHeight() + 2, getWidth() - 4, (float) (45 * open.getOutput()), 4, Color.BLACK, Color.WHITE, Color.BLACK, Color.getHSBColor(setting.getHue(), 1, 1));

            for (int max = (int) (getWidth() - 8), i = 0; i < max; i++) {
                RoundedUtil.drawRound(getX() + i + 4, (float) (getY() + FontManager.Regular.get(15).getHeight() + 2 + (45 * open.getOutput()) + 4), 2, 4, 2, Color.getHSBColor(i / (float) max, 1, 1));
            }

            float alphaSliderY = (float) (getY() + FontManager.Regular.get(15).getHeight() + 2 + (45 * open.getOutput()) + 12);
            ...
            float gradientY = getY() + Regular.get(15).getHeight() + 2;
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (RenderUtil.isHovering(getX() + getWidth() - 18, getY(), 15, FontManager.Regular.get(15).getHeight(), mouseX, mouseY)) {
            opened = !opened;
        }

        if (opened) {
            if (RenderUtil.isHovering(getX() + 4, getY() + FontManager.Regular.get(15).getHeight() + 2, getWidth() - 8, (float) (45 * open.getOutput()), mouseX, mouseY)) {
                pickingOthers = true;
            }
            if (RenderUtil.isHovering(getX() + 4, (float) (getY() + FontManager.Regular.get(15).getHeight() + 2 + (45 * open.getOutput()) + 4), getWidth() - 8, 6, mouseX, mouseY)) {
                pickingHue = true;
            }
