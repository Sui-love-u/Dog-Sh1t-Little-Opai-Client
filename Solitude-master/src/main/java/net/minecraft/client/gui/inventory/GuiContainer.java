...
import cn.gym.module.impl.player.Stealer;
...
public abstract class GuiContainer extends GuiScreen
{
    ...
    {
        if (Solitude.Instance.getModuleManager().getModule(Stealer.class).silentValue.get() && Solitude.Instance.getModuleManager().getModule(Stealer.class).getState()){
            GuiScreen guiScreen = mc.currentScreen;
            if (guiScreen instanceof GuiChest chest) {
                if (chest.lowerChestInventory != null) {
                    mc.setIngameFocus();
                    mc.currentScreen = guiScreen;
                    RenderUtil.drawLoadingCircle(this.width / 2,(float) this.height / 2.0F,-1);
