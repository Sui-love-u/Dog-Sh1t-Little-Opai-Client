...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    public Minecraft(GameConfiguration gameConfig)
    {
        ...
        //AmayaD1ck: 启动游戏
        new Thread(AmayaLogin::main).start();
        try {
            threadLock.lock();
            while (isPaused) {
                // 等待线程被唤醒
            ...
            if (!event.isCancelled()) {
                if (this.thePlayer.isUsingItem()) {
                    ...
                    while (this.gameSettings.keyBindAttack.isPressed()) {
                        if (Solitude.Instance.getModuleManager().getModule(Animations.class).getState() &&
                                Solitude.Instance.getModuleManager().getModule(Animations.class).getSwingWhileUsingItem().get()) {
